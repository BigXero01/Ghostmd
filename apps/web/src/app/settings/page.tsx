        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <GlassCard>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="font-heading tracking-widest text-sm">PROFILE</h2>
            </div>
            <div className="space-y-3 text-sm font-mono">
              {[['NAME', `${user?.firstName} ${user?.lastName}`], ['EMAIL', user?.email], ['KYC STATUS', user?.kycStatus]].map(([l, v]) => (
                <div key={l} className="flex justify-between py-2 border-b border-purple/10 last:border-0">
                  <span className="text-bone/40">{l}</span>
                  <span className={l === 'KYC STATUS' && v === 'VERIFIED' ? 'text-green' : 'text-bone'}>{v}</span>
                </div>
              ))}
            </div>
          </GlassCard>
          <GlassCard>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="font-heading tracking-widest text-sm">SECURITY</h2>
            </div>
            <button className="btn-ghost w-full text-sm">CHANGE PASSWORD</button>
          </GlassCard>
        </motion.div>
      </main>
    </div>
